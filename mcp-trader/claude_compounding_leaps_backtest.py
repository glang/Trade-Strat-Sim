...
def analyze_year_compounding_annual(year: int, starting_capital: float, 
    ...
    if year == current_year:
        exit_date = get_most_recent_trading_day(SYMBOL)
    else:
        exit_date = get_last_trading_day_of_year(SYMBOL, year)
...
def analyze_year_compounding_quarterly(year: int, starting_capital: float,
    ...
    for quarter in range(1, 5):
        ...
        exit_date = get_last_trading_day_of_quarter(SYMBOL, year, quarter)
