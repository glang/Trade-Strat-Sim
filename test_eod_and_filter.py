...
def get_bulk_eod_data_httpx(symbol: str, exp_date: str, trade_date: str, quiet: bool = False) -> Optional[Dict[str, Any]]:
    ...
    params = {
        'symbol': symbol,
        'expiration': exp_date, # Already in YYYY-MM-DD
...
def main():
    print("▶️  Testing get_bulk_eod_data and filter_itm_calls_from_bulk using httpx with verified GOOG data")
    
    # Use the verified contract data
    with open('valid_goog_contract.json', 'r') as f:
        contract_info = json.load(f)

    symbol = "GOOG"
    exp_date = contract_info['expiration']
    trade_date = contract_info['date']
    stock_price = 130.86 # GOOG price on 2023-10-02
